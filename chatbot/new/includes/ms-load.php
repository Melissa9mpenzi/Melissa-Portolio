 ...
 * manually, define `WP_PLUGIN_DIR` and `WP_PLUGIN_URL` in `config.php`.
 *
 * @access private
 * @since 3.1.0
 *
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	// If the network is defined in config.php, we can simply use that.
	...
	if ( empty( $current_site ) ) {
		/** This action is documented in includes/ms-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
	...
	if ( empty( $current_blog ) ) {
		...
		if ( $subdomain && ! defined( 'NOBLOGREDIRECT' ) ) {
			...
			$destination .= 'signup.php?new=' . str_replace( '.' . $current_site->domain, '', $domain );
		} elseif ( $subdomain ) {
			/*
