...
if ( file_exists( ABSPATH . 'config.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: config.php, 2: install.php */
...
// Check if config.php exists above the root directory but is not part of another installation.
...
<p>
	...
	<strong>
		<?php
		printf(
			/* translators: 1: wp-config-sample.php, 2: config.php */
...
<p>
			<?php
			/* translators: %s: config.php */
			printf( __( 'Unable to write to %s file.' ), '<code>config.php</code>' );
			?>
</p>
<p id="wp-config-description">
			<?php
			/* translators: %s: config.php */
			printf( __( 'You can create the %s file manually and paste the following text into it.' ), '<code>config.php</code>' );
...
<p class="configuration-rules-label"><label for="wp-config">
			<?php
			/* translators: %s: config.php */
			printf( __( 'Configuration rules for %s:' ), '<code>config.php</code>' );
			?>
	</label></p>
			...
			if ( false !== $handle ) {
				...
				if ( ! empty( $wp_config_perms ) && ! is_writable( $path_to_wp_config ) ) {
					$error_message = sprintf(
						/* translators: 1: config.php, 2: Documentation URL. */
