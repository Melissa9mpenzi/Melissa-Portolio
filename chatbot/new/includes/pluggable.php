...
if ( ! function_exists( 'auth_redirect' ) ) :
	...
	function auth_redirect() {
		...
		if ( $secure && ! is_ssl() && str_contains( $_SERVER['REQUEST_URI'], 'admin' ) ) {
		...
		if ( $user_id ) {
			...
			if ( ! $secure && get_user_option( 'use_ssl', $user_id ) && str_contains( $_SERVER['REQUEST_URI'], 'admin' ) ) {
				if ( str_starts_with( $_SERVER['REQUEST_URI'], 'http' ) ) {
					wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
					exit;
				} else {
...
if ( ! function_exists( 'wp_new_user_notification' ) ) :
	...
	function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
		 ...
		 */
		$message .= network_site_url( 'login.php?login=' . rawurlencode( $user->user_login ) . "&key=$key&action=rp", 'login' ) . "\r\n\r\n";
...
if ( ! function_exists( 'wp_salt' ) ) :
	 ...
	 * in the database and in the config.php file. The secret key in the database
	 * is randomly generated and will be appended to the secret keys in config.php.
	 *
	 * The secret keys in config.php should be updated to strong, random keys to maximize
	 * security. Below is an example of how the secret key constants are defined.
	 * Do not paste this example directly into config.php. Instead, have a
	 ...
	 * @since 2.5.0
	 *
	 * @link https://api.wordpress.org/secret-key/1.1/salt/ Create secrets for config.php
	 *
