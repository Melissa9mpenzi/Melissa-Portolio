...
class WP_Site_Health {
	...
	public function show_site_health_tab( $tab ) {
		if ( 'debug' === $tab ) {
			require_once ABSPATH . 'admin/site-health-info.php';
		}
	}
			...
			if ( wp_is_https_supported() ) {
				...
				if ( defined( 'WP_HOME' ) || defined( 'WP_SITEURL' ) ) {
					$result['description'] .= sprintf(
						'<p>%s</p>',
						sprintf(
							/* translators: 1: config.php, 2: WP_HOME, 3: WP_SITEURL */
	...
	public function get_test_background_updates() {
		...
		if ( ! class_exists( 'WP_Site_Health_Auto_Updates' ) ) {
			require_once ABSPATH . 'admin/includes/class-wp-site-health-auto-updates.php';
		}
	...
	public static function get_tests() {
		 ...
		 * to complete should run asynchronously, to avoid extended loading periods within admin.
		 *
		 * @since 5.2.0
	...
	public function wp_cron_scheduled_check() {
		// Bootstrap admin, as WP_Cron doesn't do this for us.
		require_once trailingslashit( ABSPATH ) . 'admin/includes/admin.php';
	...
	private function check_for_page_caching() {
		...
		return array(
			'advanced_cache_present'        => (
				...
				&&
				/** This filter is documented in settings.php */
