...
if ( is_admin() ) {
	...
	if ( is_network_admin() ) {
		...
		preg_match( '#/admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	} else {
		preg_match( '#/admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches );
	}
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( ! str_ends_with( $pagenow, '.php' ) ) {
			$pagenow .= '.php'; // For `Options +Multiviews`: /admin/themes/index.php (themes.php is queried).
		}
	}
} else {
