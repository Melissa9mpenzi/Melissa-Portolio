 ...
 *
 * Development mode can be set via the `WP_DEVELOPMENT_MODE` constant in `config.php`.
 ...
 * All three can be defined in config.php. By default, `WP_DEBUG` and
...
function wp_debug_mode() {
	 ...
	 * WordPress loads, usually in `config.php`.
	 *
	 * Example:
	 *
...
function wp_set_wpdb_vars() {
	...
	if ( is_wp_error( $prefix ) ) {
		...
		wp_die(
			sprintf(
				/* translators: 1: $table_prefix, 2: config.php */
