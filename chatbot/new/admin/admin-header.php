...
if ( $error_get_last && WP_DEBUG && WP_DEBUG_DISPLAY && ini_get( 'display_errors' )
	// Don't print the class for PHP notices in config.php, as they happen before WP_DEBUG takes effect,
	// and should not be displayed with the `error_reporting` level previously set in load.php.
	&& ( E_NOTICE !== $error_get_last['type'] || 'config.php' !== wp_basename( $error_get_last['file'] ) )
) {
 ...
 * 2. Not all core admin classes are filterable, notably: admin, wp-core-ui,
...
?>
<body class="admin wp-core-ui no-js <?php echo esc_attr( $admin_body_classes ); ?>">
<script type="text/javascript">
