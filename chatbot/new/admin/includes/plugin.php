...
function add_submenu_page( $parent_slug, $page_title, $menu_title, $capability, $menu_slug, $callback = '', $position = null ) {
	 ...
	 * See admin/admin.php for redirect from edit.php to tools.php.
	 */
 ...
 *
 * @since 3.0.0
 * @since 4.4.0 Function was moved into the `admin/includes/plugin.php` file.
 *
 * @param string $plugin Path to the plugin file relative to the plugins directory.
 */
...
function wp_add_privacy_policy_content( $plugin_name, $policy_text ) {
	if ( ! is_admin() ) {
		_doing_it_wrong(
			...
			sprintf(
				...
				__( 'The suggested privacy policy content should be added only in admin by using the %s (or later) action.' ),
	...
	if ( ! class_exists( 'WP_Privacy_Policy_Content' ) ) {
		require_once ABSPATH . 'admin/includes/class-wp-privacy-policy-content.php';
	}
