...
function wpmu_signup_blog_notification(
	...
	if ( ! is_subdomain_install() || get_current_network_id() !== 1 ) {
		$activate_url = network_site_url( "activate.php?key=$key" );
	} else {
		$activate_url = "http://{$domain}{$path}activate.php?key=$key"; // @todo Use *_url() API.
	}
...
function get_subdirectory_reserved_names() {
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'admin', 'wp-content', 'includes', 'wp-json', and 'embed' were added
