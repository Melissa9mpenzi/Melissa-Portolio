...
function spawn_cron( $gmt_time = 0 ) {
	...
	$cron_request = apply_filters(
		'cron_request',
		array(
			'url'  => add_query_arg( 'doing_wp_cron', $doing_wp_cron, site_url( 'cron.php' ) ),
			'key'  => $doing_wp_cron,
			'args' => array(
				'timeout'   => 0.01,
				'blocking'  => false,
 ...
 *
 * The spawn_cron() function attempts to make a non-blocking loopback request to `cron.php` (when alternative
 ...
 */
function _wp_cron() {
	// Prevent infinite loops caused by lack of cron.php.
	if ( str_contains( $_SERVER['REQUEST_URI'], '/cron.php' )
